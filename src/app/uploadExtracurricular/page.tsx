...
export default function UploadExtracurricular() {
	const [extracurriculars, setExtracurriculars] = useState<ExtracurricularFormData[]>([
		{
			id: Date.now(),
			role: "",
			organization: "",
	...
	const handleInputChange = (
		id: number,
		field: keyof Omit<ExtracurricularFormData, "id" | "image">,
		value: string
	) => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            extracurriculars.forEach((item, idx) => {
                formData.append(`extracurriculars[${idx}][role]`, item.role);
                formData.append(`extracurriculars[${idx}][organization]`, item.organization);
                formData.append(`extracurriculars[${idx}][description]`, item.description);
                formData.append(`extracurriculars[${idx}][startDate]`, item.startDate);
                formData.append(`extracurriculars[${idx}][endDate]`, item.endDate || "");
                if (item.image) {
                    formData.append(`extracurriculars[${idx}][image]`, item.image);
                }
            });
            ...
            if (!res.ok) {
                throw new Error(data.message || "Failed to upload extracurriculars");
            }
        ...
        <div className="min-h-screen bg-[#0a0a0a] text-gray-100 p-6 md:p-12 selection:bg-blue-500 selection:text-white">
            <div className="max-w-4xl mx-auto">
                <header className="mb-10 text-center">
                    ...
                    <p className="text-gray-400">Showcase your extracurricular activities</p>
                </header>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    <div className="flex justify-between items-center bg-gray-900/50 p-4 rounded-xl border border-blue-900/30 backdrop-blur-sm sticky top-4 z-10 shadow-lg shadow-blue-900/10">
                        <h2 className="text-xl font-semibold text-blue-300">
                            Extracurriculars List ({extracurriculars.length})
                        </h2>
                        <button
                            type="button"
                            ...
                            >
                                <div className="absolute top-4 right-4">
                                    {extracurriculars.length > 1 && (
                                        <button
                                            type="button"
                                            onClick={() => removeExtracurricular(item.id)}
                                            className="text-gray-500 hover:text-red-400 transition-colors p-2"
                                            title="Remove Extracurricular"
                                        >
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                                                className="h-6 w-6"
                                ...
                                <div className="mb-6 flex items-center gap-3">
                                    ...
                                    <h3 className="text-lg font-medium text-gray-200">Extracurricular Details</h3>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="col-span-2 md:col-span-1">
                                        <label className="block text-sm font-medium text-gray-400 mb-1">
                                            Organization <span className="text-blue-500">*</span>
                                        </label>
                                        <input
                                            type="text"
                                            required
                                            ...
                                            onChange={(e) =>
                                                handleInputChange(item.id, "organization", e.target.value)
                                            }
                                            className="w-full bg-[#1a1a1a] border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all placeholder-gray-600"
                                    ...
                                    <div className="col-span-2">
                                        ...
                                        <textarea
                                            ...
                                            placeholder="Describe your extracurricular activity..."
                                        />
                                    </div>
                                    ...
                                    <div className="col-span-2">
                                        <label className="block text-sm font-medium text-gray-400 mb-1">
                                            Image <span className="text-blue-500">*</span>
                                        </label>
                    ...
                    <div className="flex justify-end pt-6 border-t border-gray-800">
                        ...
                        >
                            {isSubmitting ? "Uploading..." : "Submit All Extracurriculars"}
                        </button>
                    </div>
                </form>
            </div>
